...
import { Box, Typography, Grid, Card, CardContent, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Select, MenuItem, FormControl, InputLabel, Switch, FormControlLabel, Tabs, Tab, Avatar, Tooltip, Paper, Alert } from '@mui/material';
import { PlayArrow, Stop, Settings, Delete, Add, Refresh, ShowChart, TrendingUp, TrendingDown, Speed, AccountBalanceWallet, CheckCircle, Error, Warning } from '@mui/icons-material';
// ============================================================================
...
const Bots = () => {
    ...
    const [selectedDexs, setSelectedDexs] = useState(TOP_DEXS);
    const [selectedCexs, setSelectedCexs] = useState([]);
    ...
    return (<Box sx={{ p: 3 }}>
      ...
      <TableContainer component={Paper}>
        <Table>
          ...
          <TableBody>
            ...
            .map((bot) => (<TableRow key={bot.id} hover>
                  <TableCell>
                    <Box>
                  ...
                  <TableCell>
                    <Chip label={BOT_TYPES.find(t => t.value === bot.type)?.label || bot.type} size="small" variant="outlined"/>
                  </TableCell>
                  <TableCell>
                    <Chip icon={bot.status === 'running' ? <CheckCircle /> :
                bot.status === 'error' ? <Error /> : <Warning />} label={bot.status.toUpperCase()} color={getStatusColor(bot.status)} size="small"/>
                  </TableCell>
                  <TableCell>
                ...
                fontWeight: 'bold'
                  ...
                  <TableCell>
                    <Chip label={`${bot.avgLatency}μs`} color={getLatencyColor(bot.avgLatency)} size="small"/>
                  </TableCell>
                  <TableCell>
                    <Tooltip title={`${bot.connectedDexs} DEXs, ${bot.connectedCexs} CEXs`}>
                      <Chip label={`${bot.connectedDexs}/${bot.connectedCexs}`} size="small" variant="outlined"/>
                    </Tooltip>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', gap: 0.5 }}>
                      {bot.status === 'running' ? (<Tooltip title="Stop Bot">
                          <IconButton color="error" size="small" onClick={() => handleStopBot(bot.id)}>
                            <Stop />
                          </IconButton>
                        </Tooltip>) : (<Tooltip title="Start Bot">
                          <IconButton color="success" size="small" onClick={() => handleStartBot(bot.id)}>
                            <PlayArrow />
                          </IconButton>
                        </Tooltip>)}
                      <Tooltip title="Settings">
                        <IconButton color="info" size="small" onClick={() => handleOpenSettings(bot)}>
                          <Settings />
                        </IconButton>
                      </Tooltip>
                      <Tooltip title="Delete">
                        <IconButton color="error" size="small" onClick={() => handleDeleteBot(bot.id)}>
                          <Delete />
                        </IconButton>
                      </Tooltip>
                    </Box>
      ...
      <Dialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField fullWidth label="Bot Name" value={newBotName} onChange={(e) => setNewBotName(e.target.value)} placeholder="e.g., ETH Market Maker"/>
            </Grid>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                <InputLabel>Bot Type</InputLabel>
                <Select value={newBotType} label="Bot Type" onChange={(e) => setNewBotType(e.target.value)}>
                  {BOT_TYPES.map((type) => (<MenuItem key={type.value} value={type.value}>
                      <Box>
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select value={latencyTarget} label="Latency Target (μs)" onChange={(e) => setLatencyTarget(Number(e.target.value))}>
            ...
            <Grid item xs={12} md={6}>
              <TextField fullWidth type="number" label="Max Position (USD)" value={maxPosition} onChange={(e) => setMaxPosition(Number(e.target.value))}/>
            </Grid>
            <Grid item xs={12} md={6}>
              <TextField fullWidth type="number" label="Max Daily Loss (USD)" value={maxDailyLoss} onChange={(e) => setMaxDailyLoss(Number(e.target.value))}/>
            </Grid>
            <Grid item xs={12}>
              <FormControlLabel control={<Switch checked={selectedDexs.length === TOP_DEXS.length}/>} label="Connect to All 20 Top DEXs" onChange={(_, checked) => {
            if (checked)
                setSelectedDexs(TOP_DEXS);
            else
                setSelectedDexs([]);
            ...
            <Grid item xs={12}>
              <FormControlLabel control={<Switch checked={selectedCexs.length === 200}/>} label="Connect to All 200 CEXs" onChange={(_, checked) => {
            if (checked)
                setSelectedCexs(Array.from({ length: 200 }, (_, i) => `cex_${i}`));
            else
                setSelectedCexs([]);
        }}/>
            </Grid>
      ...
      <Dialog open={settingsDialogOpen} onClose={() => setSettingsDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {selectedBot && (<Grid container spacing={2} sx={{ mt: 1 }}>
              <Grid item xs={12}>
              ...
              <Grid item xs={12} md={6}>
                <TextField fullWidth type="number" label="Max Position (USD)" defaultValue={maxPosition}/>
              </Grid>
              <Grid item xs={12} md={6}>
                <TextField fullWidth type="number" label="Max Daily Loss (USD)" defaultValue={maxDailyLoss}/>
              </Grid>
              <Grid item xs={12}>
