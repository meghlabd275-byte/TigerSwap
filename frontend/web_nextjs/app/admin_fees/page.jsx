...
import { Box, Typography, Card, CardContent, Button, TextField, Tabs, Tab, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Select, MenuItem, FormControl, InputLabel, Switch, FormControlLabel, Grid, LinearProgress } from '@mui/material';
import { MonetizationOn, Receipt, TrendingUp, Add, Edit, Delete, Refresh, CheckCircle, Error as ErrorIcon } from '@mui/icons-material';
...
export default function AdminFeesPage() {
    ...
    // Actions
    // ============================================================================
    const handleEditFee = useCallback((fee) => {
        setSelectedFee(fee);
        setFormData({
            name: fee.name,
            type: fee.type,
    ...
    const handleSaveFee = useCallback(async () => {
        ...
        try {
            await new Promise(resolve => setTimeout(resolve, 500));
            if (selectedFee) {
                setFeeConfigs(feeConfigs.map(f => f.id === selectedFee.id ? { ...f, ...formData, updatedAt: Date.now() } : f));
    ...
    const handleToggleFee = useCallback(async (feeId) => {
        setFeeConfigs(feeConfigs.map(f => f.id === feeId ? { ...f, isActive: !f.isActive } : f));
    }, [feeConfigs]);
    // ============================================================================
    // Helper Functions
    // ============================================================================
    ...
    return (<Box sx={{ p: 3 }}>
      ...
      {/* Revenue Stats */}
      {revenueStats && (<Grid container spacing={3} sx={{ mb: 3 }}>
      ...
      {activeTab === 0 && (<TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {feeConfigs.map(fee => (<TableRow key={fee.id}>
                  <TableCell>{fee.name}</TableCell>
                  <TableCell>
                    <Chip label={fee.type} size="small" color={fee.type === 'swap' ? 'primary' : fee.type === 'bot' ? 'secondary' : 'default'}/>
                  </TableCell>
          ...
          <Table>
            ...
            <TableBody>
              {transactions.map(tx => (<TableRow key={tx.id}>
                  <TableCell>
                    <Chip label={tx.type} size="small" color={tx.type === 'swap' ? 'primary' : tx.type === 'bot' ? 'secondary' : 'default'}/>
                  </TableCell>
                  ...
                  <TableCell>
                    <Chip label={tx.status} size="small" color={tx.status === 'collected' ? 'success' : tx.status === 'distributed' ? 'info' : 'warning'}/>
                  </TableCell>
                </TableRow>))}
            </TableBody>
          </Table>
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                }).map(([name, value]) => (<Box key={name} sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          ...
          <Grid item xs={12} md={6}>
            <Card>
              <CardContent>
                ...
                ].map(item => (<Box key={item.label} sx={{ mb: 2 }}>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <FormControl fullWidth>
              ...
              <Select value={formData.type} label="Fee Type" onChange={(e) => setFormData({ ...formData, type: e.target.value })}>
                {FEE_TYPES.map(ft => (<MenuItem key={ft.value} value={ft.value}>{ft.label}</MenuItem>))}
              </Select>
            </FormControl>
            <FormControl fullWidth>
            ...
            <FormControl fullWidth>
              ...
              <Select value={formData.feeType} label="Calculation Type" onChange={(e) => setFormData({ ...formData, feeType: e.target.value })}>
                <MenuItem value="percentage">Percentage (%)</MenuItem>
                <MenuItem value="flat">Flat Fee ($)</MenuItem>
                <MenuItem value="tiered">Tiered</MenuItem>
              </Select>
            </FormControl>
            <TextField fullWidth label={formData.feeType === 'percentage' ? 'Fee (%)' : 'Fee (USD)'} type="number" value={formData.value} onChange={(e) => setFormData({ ...formData, value: parseFloat(e.target.value) })}/>
            {formData.feeType === 'tiered' && (<>
                <TextField fullWidth label="Min Value" type="number" value={formData.minValue} onChange={(e) => setFormData({ ...formData, minValue: parseFloat(e.target.value) })}/>
                <TextField fullWidth label="Max Value" type="number" value={formData.maxValue} onChange={(e) => setFormData({ ...formData, maxValue: parseFloat(e.target.value) })}/>
              </>)}
