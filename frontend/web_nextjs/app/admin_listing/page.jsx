...
import { Box, Typography, Card, CardContent, Button, Tabs, Tab, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Switch, Grid, Avatar, Divider, LinearProgress, Tooltip } from '@mui/material';
import { ListAlt, Refresh, CheckCircle, Error as ErrorIcon, Schedule, Verified, TrendingUp, TrendingDown, Visibility, Star, ThumbUp, ThumbDown } from '@mui/icons-material';
// ============================================================================
// Component
// ============================================================================
export default function AdminListingPage() {
    ...
    const handleApprove = useCallback(async (requestId) => {
        ...
        try {
            await new Promise(resolve => setTimeout(resolve, 500));
            setRequests(requests.map(r => r.id === requestId ? { ...r, status: 'approved', resolvedAt: Date.now() } : r));
    ...
    const handleReject = useCallback(async (requestId) => {
        ...
        try {
            await new Promise(resolve => setTimeout(resolve, 500));
            setRequests(requests.map(r => r.id === requestId ? { ...r, status: 'rejected', resolvedAt: Date.now() } : r));
    ...
    const handleFeature = useCallback(async (tokenId, featured) => {
        setTokens(tokens.map(t => t.id === tokenId ? { ...t, featured } : t));
    }, [tokens]);
    const handleVerify = useCallback(async (tokenId, verified) => {
        setTokens(tokens.map(t => t.id === tokenId ? { ...t, verified } : t));
    }, [tokens]);
    ...
    return (<Box sx={{ p: 3 }}>
      ...
      {activeTab === 0 && (<TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {requests.filter(r => r.status === 'pending').map(request => (<TableRow key={request.id}>
                  <TableCell>
                    <Box>
                  ...
                  <TableCell>
                    <Chip label={request.kycStatus.toUpperCase()} size="small" color={request.kycStatus === 'full' ? 'success' : request.kycStatus === 'basic' ? 'warning' : 'default'}/>
                  </TableCell>
                  <TableCell>
                    <Chip label={request.auditStatus.toUpperCase()} size="small" color={request.auditStatus === 'approved' ? 'success' : request.auditStatus === 'pending' ? 'warning' : request.auditStatus === 'failed' ? 'error' : 'default'}/>
                  </TableCell>
          ...
          <Table>
            ...
            <TableBody>
              {tokens.map(token => (<TableRow key={token.id}>
                  ...
                  <TableCell>
                    <Switch checked={token.verified} onChange={(e) => handleVerify(token.id, e.target.checked)} size="small"/>
                  </TableCell>
                  <TableCell>
                    <Switch checked={token.featured} onChange={(e) => handleFeature(token.id, e.target.checked)} size="small"/>
                  </TableCell>
                </TableRow>))}
            </TableBody>
          </Table>
      ...
      {activeTab === 2 && (<Grid container spacing={3}>
          {tokens.filter(t => t.featured).map(token => (<Grid item xs={12} md={4} key={token.id}>
              <Card>
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                    ...
                    {token.verified && (<Chip icon={<Verified />} label="Verified" size="small" color="primary"/>)}
                  </Box>
                  <Divider sx={{ my: 2 }}/>
