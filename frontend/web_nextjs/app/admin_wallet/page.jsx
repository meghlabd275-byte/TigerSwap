...
import { Box, Typography, Card, CardContent, Button, TextField, Tabs, Tab, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Select, MenuItem, FormControl, InputLabel, Grid, Divider, List, ListItem, ListItemText, ListItemIcon, LinearProgress } from '@mui/material';
import { AccountBalanceWallet, Add, Delete, Refresh, Visibility, ContentCopy, Send, MoveToInbox, SwapHoriz, Key, Security, Warning, CheckCircle, Lock, AccountTree, Hexagon } from '@mui/icons-material';
// ============================================================================
// Component
// ============================================================================
export default function AdminWalletPage() {
    ...
    const handleFreezeWallet = useCallback(async (walletId) => {
        setWallets(wallets.map(w => w.id === walletId ? { ...w, status: w.status === 'active' ? 'frozen' : 'active' } : w));
    }, [wallets]);
    const handleViewWallet = useCallback((wallet) => {
    ...
    return (<Box sx={{ p: 3 }}>
      ...
      {/* Stats */}
      {stats && (<Grid container spacing={3} sx={{ mb: 3 }}>
      ...
      {activeTab === 0 && (<Grid container spacing={3}>
          {wallets.map(wallet => (<Grid item xs={12} md={6} key={wallet.id}>
              <Card>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                    ...
                    <Chip label={wallet.status.toUpperCase()} size="small" color={wallet.status === 'active' ? 'success' : 'error'}/>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                    ...
                    <Button size="small" color={wallet.status === 'active' ? 'warning' : 'success'} onClick={() => handleFreezeWallet(wallet.id)}>
            ...
            <TableBody>
              {transactions.map(tx => {
                ...
                return (<TableRow key={tx.id}>
                    <TableCell>
                      <Chip label={tx.type.toUpperCase()} size="small" color={tx.type === 'send' ? 'error' : tx.type === 'receive' ? 'success' : 'primary'} icon={tx.type === 'send' ? <Send /> : tx.type === 'receive' ? <MoveToInbox /> : <SwapHoriz />}/>
                    </TableCell>
                    ...
                    <TableCell>
                      <Chip label={tx.status.toUpperCase()} size="small" color={tx.status === 'confirmed' ? 'success' : tx.status === 'pending' ? 'warning' : 'error'}/>
                    </TableCell>
                  </TableRow>);
            })}
            </TableBody>
      ...
      {/* Multi-Sig Tab */}
      {activeTab === 2 && (<Grid container spacing={3}>
          {wallets.filter(w => w.type === 'multi_sig').map(wallet => (<Grid item xs={12} md={6} key={wallet.id}>
              <Card>
                <CardContent>
                  ...
                  {wallet.signers?.map((signer, i) => (<Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
      ...
      {/* Security Tab */}
      {activeTab === 3 && (<Grid container spacing={3}>
      ...
      <Dialog open={createDialogOpen} onClose={() => setCreateDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField fullWidth label="Wallet Name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}/>
            <FormControl fullWidth>
              <InputLabel>Wallet Type</InputLabel>
              <Select value={formData.type} label="Wallet Type" onChange={(e) => setFormData({ ...formData, type: e.target.value })}>
            ...
            <FormControl fullWidth>
              ...
              <Select value={formData.chainId} label="Chain" onChange={(e) => setFormData({ ...formData, chainId: e.target.value })}>
            ...
            {formData.type === 'multi_sig' && (<>
                ...
                {formData.signers.map((signer, i) => (<TextField key={i} fullWidth label={`Signer ${i + 1} Address`} value={signer} onChange={(e) => {
                ...
                <TextField fullWidth label="Threshold" type="number" value={formData.threshold} onChange={(e) => setFormData({ ...formData, threshold: parseInt(e.target.value) })}/>
              </>)}
          </Box>
        </DialogContent>
        <DialogActions>
