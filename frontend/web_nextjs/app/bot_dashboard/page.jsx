...
import { Box, Typography, Card, CardContent, Button, TextField, Tabs, Tab, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Select, MenuItem, FormControl, InputLabel, Switch, FormControlLabel, Slider, Grid, Divider, LinearProgress } from '@mui/material';
import { Speed, ShowChart, TrendingUp, TrendingDown, Settings, Add, PlayArrow, Stop, Pause, Delete, Refresh, Build, MonetizationOn, CheckCircle, Error as ErrorIcon, People, Store, Dashboard, History } from '@mui/icons-material';
...
export default function BotDashboard() {
    ...
    const handlePauseBot = useCallback(async (botId) => {
        setLoading(true);
        try {
            await new Promise(resolve => setTimeout(resolve, 300));
    ...
    return (<Box sx={{ p: 3 }}>
      ...
      <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          {currentUser?.role === 'admin' && (<Button variant="contained" startIcon={<People />} onClick={() => setCreateUserDialog(true)}>
              Add User
            </Button>)}
      ...
      <Tabs value={activeTab} onChange={(_, v) => setActiveTab(v)} sx={{ mb: 3 }}>
        ...
        {currentUser?.role === 'admin' && (<Tab label="Users" icon={<People />} iconPosition="start"/>)}
      ...
      {activeTab === 0 && (<TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {bots.map(bot => (<TableRow key={bot.id}>
                  <TableCell>{bot.name}</TableCell>
                  <TableCell>
                    <Chip label={bot.type.replace('_', ' ')} size="small" icon={BOT_TYPES.find(t => t.value === bot.type)?.icon}/>
                  </TableCell>
                  <TableCell>
                    <Chip label={bot.status} size="small" color={bot.status === 'running' ? 'success' : bot.status === 'paused' ? 'warning' : 'default'}/>
                  </TableCell>
                  ...
                  <TableCell>
                    <Box sx={{ display: 'flex', gap: 1 }}>
                      {bot.status === 'stopped' ? (<IconButton size="small" color="success" onClick={() => handleStartBot(bot.id)}>
                          <PlayArrow />
                        </IconButton>) : (<IconButton size="small" color="warning" onClick={() => handlePauseBot(bot.id)}>
                          <Pause />
                        </IconButton>)}
      ...
      {/* Transactions Tab */}
      {activeTab === 1 && (<TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {transactions.map(tx => (<TableRow key={tx.id}>
                  ...
                  <TableCell>
                    <Chip label={tx.status} size="small" color={tx.status === 'success' ? 'success' : 'error'} icon={tx.status === 'success' ? <CheckCircle /> : <ErrorIcon />}/>
                  </TableCell>
                </TableRow>))}
            </TableBody>
          </Table>
      ...
      {/* Exchanges Tab */}
      {activeTab === 2 && (<TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>Name</TableCell>
            ...
            <TableBody>
              {exchanges.map(ex => (<TableRow key={ex.id}>
                  <TableCell>{ex.name}</TableCell>
                  <TableCell>
                    <Chip label={ex.isActive ? 'Active' : 'Inactive'} size="small" color={ex.isActive ? 'success' : 'default'}/>
                  </TableCell>
                  ...
                  <TableCell>
                    <FormControlLabel control={<Switch checked={ex.isActive} onChange={() => {
                        setExchanges(exchanges.map(e => e.id === ex.id ? { ...e, isActive: !e.isActive } : e));
      ...
      {activeTab === 3 && currentUser?.role === 'admin' && (<TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>Name</TableCell>
            ...
            <TableBody>
              {users.map(user => (<TableRow key={user.id}>
                  ...
                  <TableCell>
                    <Chip label={user.role} size="small" color={user.role === 'admin' ? 'error' : user.role === 'operator' ? 'warning' : 'primary'}/>
                  </TableCell>
                  ...
                  <TableCell>
                    {user.role !== 'admin' && (<IconButton size="small" onClick={() => handleDeleteUser(user.id)}>
                        <Delete />
                      </IconButton>)}
                  </TableCell>
                </TableRow>))}
      ...
      {activeTab === 4 && (<Card>
          <CardContent>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <FormControlLabel control={<Switch defaultChecked/>} label="Require Token Verification"/>
              </Grid>
              <Grid item xs={12}>
                <FormControlLabel control={<Switch defaultChecked/>} label="Require KYC"/>
              </Grid>
            </Grid>
          </CardContent>
        </Card>)}
      ...
      <Dialog open={createBotDialog} onClose={() => setCreateBotDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField fullWidth label="Bot Name" value={newBot.name} onChange={(e) => setNewBot({ ...newBot, name: e.target.value })}/>
            <FormControl fullWidth>
              <InputLabel>Bot Type</InputLabel>
              <Select value={newBot.type} label="Bot Type" onChange={(e) => setNewBot({ ...newBot, type: e.target.value })}>
                {BOT_TYPES.map(bt => (<MenuItem key={bt.value} value={bt.value}>
            ...
            <TextField fullWidth label="Min Investment (USD)" type="number" value={newBot.minInvestment} onChange={(e) => setNewBot({ ...newBot, minInvestment: parseInt(e.target.value) })}/>
            <TextField fullWidth label="Max Investment (USD)" type="number" value={newBot.maxInvestment} onChange={(e) => setNewBot({ ...newBot, maxInvestment: parseInt(e.target.value) })}/>
            <TextField fullWidth label="Target APY (%)" type="number" value={newBot.targetApy} onChange={(e) => setNewBot({ ...newBot, targetApy: parseInt(e.target.value) })}/>
            <Box>
              <Typography variant="subtitle2" gutterBottom>Risk Level: {newBot.riskLevel}</Typography>
              <Slider value={newBot.riskLevel} onChange={(_, v) => setNewBot({ ...newBot, riskLevel: v })} min={1} max={10} marks={[
            { value: 1, label: 'Low' },
      ...
      <Dialog open={createUserDialog} onClose={() => setCreateUserDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <TextField fullWidth label="Wallet Address" value={newUser.address} onChange={(e) => setNewUser({ ...newUser, address: e.target.value })}/>
            <FormControl fullWidth>
