...
import { Box, Typography, Card, CardContent, Button, TextField, Select, MenuItem, FormControl, Chip, CircularProgress, Snackbar, Alert, IconButton, Stepper, Step, StepLabel, Divider, InputAdornment } from '@mui/material';
import { SwapHoriz, ArrowForward, CheckCircle, Speed, Security } from '@mui/icons-material';
// ============================================================================
// Constants
// ============================================================================
const CHAINS = [
...
// Mock Data Generator
// ============================================================================
function generateBridgeQuote(fromChain, toChain, token, amount) {
    ...
    const handleBridge = async () => {
        ...
        try {
            ...
            for (let i = 1; i <= 12; i++) {
                ...
                setTransferHistory(prev => prev.map(tx => tx.id === newTransfer.id
    ...
    return (<Box sx={{ minHeight: '100vh', bgcolor: '#0a0a14', p: 3 }}>
      <Box sx={{ maxWidth: 1200, mx: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
          <Button variant={activeTab === 0 ? 'contained' : 'outlined'} onClick={() => setActiveTab(0)} sx={{ bgcolor: activeTab === 0 ? '#00d4aa' : 'transparent', color: activeTab === 0 ? 'black' : 'white', borderColor: '#3a3a4e' }}>
            Bridge
          </Button>
          <Button variant={activeTab === 1 ? 'contained' : 'outlined'} onClick={() => setActiveTab(1)} sx={{ bgcolor: activeTab === 1 ? '#00d4aa' : 'transparent', color: activeTab === 1 ? 'black' : 'white', borderColor: '#3a3a4e' }}>
            History
          </Button>
        </Box>
            ...
            <Card sx={{ bgcolor: '#1a1a2e', borderRadius: 3 }}>
              <CardContent sx={{ p: 4 }}>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                    <FormControl fullWidth size="small">
                      <Select value={fromChain} onChange={(e) => setFromChain(e.target.value)} sx={{ color: 'white', bgcolor: '#2a2a3e', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                        {CHAINS.map(chain => (<MenuItem key={chain.id} value={chain.id}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <TextField size="small" type="number" placeholder="0.0" value={amount} onChange={(e) => setAmount(e.target.value)} InputProps={{
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', my: 2 }}>
                  <IconButton onClick={handleSwitchChains} sx={{
                bgcolor: '#2a2a3e',
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <FormControl fullWidth size="small">
                    <Select value={toChain} onChange={(e) => setToChain(e.target.value)} sx={{ color: 'white', bgcolor: '#2a2a3e', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                      {CHAINS.filter(c => c.id !== fromChain).map(chain => (<MenuItem key={chain.id} value={chain.id}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <span>{chain.icon}</span>
                            <span>{chain.name}</span>
                          </Box>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    {TOKENS.map(t => (<Chip key={t.symbol} label={<Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                            <span>{t.icon}</span>
                            <span>{t.symbol}</span>
                          </Box>} onClick={() => setToken(t.symbol)} sx={{
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mb: 3 }}>
                  {quote?.availableRoutes.slice(0, 3).map(route => (<Box key={route.id} onClick={() => setSelectedRoute(route.id)} sx={{
                    p: 2,
                    borderRadius: 2,
                ...
                {/* Bridge Button */}
                <Button fullWidth variant="contained" size="large" onClick={handleBridge} disabled={!quote || loading || activeStep > 0} startIcon={loading ? <CircularProgress size={20} sx={{ color: 'black' }}/> : <ArrowForward />} sx={{
                bgcolor: '#00d4aa',
                color: 'black',
                py: 1.5,
        ...
        <Card sx={{ bgcolor: '#1a1a2e', borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {transferHistory.map(tx => (<Box key={tx.id} sx={{
                    p: 3,
                    bgcolor: '#2a2a3e',
                    borderRadius: 2,
                    display: 'flex',
                    ...
                    <Box>
                      ...
                      <Typography variant="caption" sx={{ color: '#9ca3af' }}>
                        ...
                        {tx.sourceTxHash && (<> • Source: <a href={`${CHAINS.find(c => c.id === tx.fromChain)?.explorerUrl}/tx/${tx.sourceTxHash}`} target="_blank" rel="noopener" style={{ color: '#00d4aa' }}>{formatAddress(tx.sourceTxHash)}</a></>)}
                      </Typography>
                      {tx.status === 'confirming' && (<Box sx={{ mt: 1 }}>
                          <Typography variant="caption" sx={{ color: '#ff9800' }}>
                            Confirming: {tx.confirmations}/{tx.requiredConfirmations} blocks
                          </Typography>
                        </Box>)}
                    </Box>
                    {tx.destTxHash && (<Button size="small" href={`${CHAINS.find(c => c.id === tx.toChain)?.explorerUrl}/tx/${tx.destTxHash}`} target="_blank" sx={{ color: '#00d4aa' }}>
                        View on Dest
                      </Button>)}
                  </Box>))}
              </Box>
        ...
        <Box sx={{ mt: 4 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            {CHAINS.map(chain => (<Card key={chain.id} sx={{ bgcolor: '#1a1a2e', borderRadius: 2, minWidth: 150 }}>
