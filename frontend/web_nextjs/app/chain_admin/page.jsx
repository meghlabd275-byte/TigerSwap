...
import { Box, Typography, Card, CardContent, Button, TextField, Tabs, Tab, Chip, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Dialog, DialogTitle, DialogContent, DialogActions, Alert, Select, MenuItem, FormControl, InputLabel, Switch, FormControlLabel, Grid, Divider, LinearProgress, Tooltip } from '@mui/material';
import { AccountTree, Lan, Storage, Cloud, Add, Refresh, Speed, ShowChart, CloudQueue, Dns, SwapHoriz } from '@mui/icons-material';
// ============================================================================
// Constants
// ============================================================================
const CHAIN_TYPES = [
...
export default function ChainAdminPanel() {
    ...
    const [tokenDeployments, setTokenDeployments] = useState([]);
    ...
    const handleUpdateChainStatus = useCallback(async (chainId, status) => {
        setLoading(true);
        try {
    ...
    return (<Box sx={{ p: 3 }}>
      ...
      {activeTab === 0 && (<Grid container spacing={3}>
          {chains.map(chain => (<Grid item xs={12} md={6} key={chain.id}>
              <Card>
                <CardContent>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                    ...
                    <Chip label={chain.status} size="small" color={chain.status === 'active' ? 'success' : chain.status === 'paused' ? 'warning' : 'default'}/>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                    {chain.status === 'active' ? (<Button size="small" variant="outlined" color="warning" onClick={() => handleUpdateChainStatus(chain.id, 'paused')}>
                        Pause
                      </Button>) : (<Button size="small" variant="outlined" color="success" onClick={() => handleUpdateChainStatus(chain.id, 'active')}>
                        Activate
                      </Button>)}
      ...
      {activeTab === 1 && (<Box>
          ...
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {validators.map(validator => (<TableRow key={validator.id}>
                    ...
                    <TableCell>
                      <Chip label={validator.status} size="small" color={validator.status === 'active' ? 'success' : 'default'}/>
                    </TableCell>
                    <TableCell align="right">{validator.performance.toFixed(2)}%</TableCell>
                    <TableCell>
                      {validator.chains.map(c => (<Chip key={c} label={c} size="small" sx={{ mr: 0.5 }}/>))}
                    </TableCell>
                  </TableRow>))}
              </TableBody>
            </Table>
      ...
      {activeTab === 2 && (<Box>
          ...
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {bridges.map(bridge => (<TableRow key={bridge.id}>
                    <TableCell>{bridge.name}</TableCell>
                    <TableCell>{bridge.sourceChain}</TableCell>
                    <TableCell>{bridge.destChain}</TableCell>
                    <TableCell>
                      <Chip label={bridge.status} size="small" color={bridge.status === 'active' ? 'success' : 'default'}/>
                    </TableCell>
      ...
      {activeTab === 3 && (<TableContainer component={Paper}>
          <Table>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {tokenDeployments.map(token => (<TableRow key={token.id}>
                  ...
                  <TableCell>
                    <Chip label={token.status} size="small" color={token.status === 'deployed' ? 'success' : token.status === 'pending' ? 'warning' : 'error'}/>
                  </TableCell>
                </TableRow>))}
            </TableBody>
          </Table>
      ...
      {/* Metrics Tab */}
      {activeTab === 4 && (<Grid container spacing={3}>
          {Object.entries(metrics).map(([chainId, m]) => (<Grid item xs={12} md={6} key={chainId}>
              <Card>
                <CardContent>
      ...
      <Dialog open={addChainDialog} onClose={() => setAddChainDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField fullWidth label="Chain Name" value={newChain.name} onChange={(e) => setNewChain({ ...newChain, name: e.target.value })}/>
            <FormControl fullWidth>
              <InputLabel>Chain Type</InputLabel>
              <Select value={newChain.type} label="Chain Type" onChange={(e) => setNewChain({ ...newChain, type: e.target.value })}>
                {CHAIN_TYPES.map(ct => (<MenuItem key={ct.value} value={ct.value}>
            ...
            <TextField fullWidth label="Chain ID" type="number" value={newChain.chainId} onChange={(e) => setNewChain({ ...newChain, chainId: parseInt(e.target.value) })}/>
            <TextField fullWidth label="RPC URL" value={newChain.rpcUrl} onChange={(e) => setNewChain({ ...newChain, rpcUrl: e.target.value })}/>
            <TextField fullWidth label="Explorer URL" value={newChain.explorerUrl} onChange={(e) => setNewChain({ ...newChain, explorerUrl: e.target.value })}/>
            <FormControlLabel control={<Switch checked={newChain.isTestnet} onChange={(e) => setNewChain({ ...newChain, isTestnet: e.target.checked })}/>} label="Testnet"/>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={addValidatorDialog} onClose={() => setAddValidatorDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField fullWidth label="Validator Name" value={newValidator.name} onChange={(e) => setNewValidator({ ...newValidator, name: e.target.value })}/>
            <TextField fullWidth label="Validator Address" value={newValidator.address} onChange={(e) => setNewValidator({ ...newValidator, address: e.target.value })}/>
            <TextField fullWidth label="Endpoint URL" value={newValidator.endpoint} onChange={(e) => setNewValidator({ ...newValidator, endpoint: e.target.value })}/>
            <TextField fullWidth label="Stake Amount" type="number" value={newValidator.stake} onChange={(e) => setNewValidator({ ...newValidator, stake: parseInt(e.target.value) })}/>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={addBridgeDialog} onClose={() => setAddBridgeDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField fullWidth label="Bridge Name" value={newBridge.name} onChange={(e) => setNewBridge({ ...newBridge, name: e.target.value })}/>
            <FormControl fullWidth>
              <InputLabel>Source Chain</InputLabel>
              <Select value={newBridge.sourceChain} label="Source Chain" onChange={(e) => setNewBridge({ ...newBridge, sourceChain: e.target.value })}>
                {chains.map(c => (<MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>))}
              </Select>
            </FormControl>
            <FormControl fullWidth>
              <InputLabel>Destination Chain</InputLabel>
              <Select value={newBridge.destChain} label="Destination Chain" onChange={(e) => setNewBridge({ ...newBridge, destChain: e.target.value })}>
                {chains.map(c => (<MenuItem key={c.id} value={c.id}>{c.name}</MenuItem>))}
              </Select>
            </FormControl>
            <TextField fullWidth label="Min Amount" type="number" value={newBridge.minAmount} onChange={(e) => setNewBridge({ ...newBridge, minAmount: parseInt(e.target.value) })}/>
            <TextField fullWidth label="Max Amount" type="number" value={newBridge.maxAmount} onChange={(e) => setNewBridge({ ...newBridge, maxAmount: parseInt(e.target.value) })}/>
            <TextField fullWidth label="Fee (%)" type="number" value={newBridge.fee} onChange={(e) => setNewBridge({ ...newBridge, fee: parseFloat(e.target.value) })}/>
          </Box>
        </DialogContent>
        <DialogActions>
