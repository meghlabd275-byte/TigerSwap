...
function generateCandlestickData(basePrice, days, volatility = 0.02) {
    const data = [];
...
export default function PriceChart({ tokenPair = 'ETH_USDC', tokenIn, tokenOut, height = 400, showVolume = true, showOrderBook = false }) {
    ...
    return (<Card sx={{ bgcolor: '#1a1a2e', borderRadius: 3 }}>
      <CardContent>
        {/* Header */}
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          ...
          <ToggleButtonGroup value={timeframe} exclusive onChange={handleTimeframeChange} size="small" sx={{
        ...
        {/* Order Book Preview */}
        {showOrderBook && (<Box sx={{ mt: 2, display: 'flex', gap: 2 }}>
            <Box sx={{ flex: 1 }}>
              <Typography variant="body2" sx={{ color: 'gray', mb: 1 }}>Bids</Typography>
              {[0.5, 0.3, 0.2, 0.1, 0.05].map((pct, i) => (<Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              {[0.05, 0.1, 0.2, 0.3, 0.5].map((pct, i) => (<Box key={i} sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
...
//# sourceMappingURL=PriceChart.jsx.map