...
export default function TradingViewWidget({ symbol = 'BINANCE:ETHUSDC', interval = '60', theme = 'dark', height = 500, width = '100%', locale = 'en', autosize = true, }) {
    ...
    const handleIntervalChange = (_event, newInterval) => {
        if (newInterval) {
            setSelectedInterval(newInterval);
        }
    };
    return (<Card sx={{ bgcolor: theme === 'dark' ? '#1a1a2e' : '#ffffff', borderRadius: 3, border: `1px solid ${theme === 'dark' ? '#2a2a3e' : '#e0e0e0'}` }}>
      <CardContent>
        {/* Header Controls */}
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <FormControl size="small" sx={{ minWidth: 120 }}>
              ...
              <Select value={selectedSymbol} label="Pair" onChange={handleSymbolChange} sx={{
            color: theme === 'dark' ? 'white' : 'black',
            '.MuiOutlinedInput-notchedOutline': {
                borderColor: theme === 'dark' ? '#2a2a3e' : '#e0e0e0',
            },
        }}>
                {symbols.map((s) => (<MenuItem key={s.value} value={s.value}>
                    {s.label}
                  </MenuItem>))}
              </Select>
            </FormControl>
          ...
          <ToggleButtonGroup value={selectedInterval} exclusive onChange={handleIntervalChange} size="small" sx={{
            ...
            {intervals.map((i) => (<ToggleButton key={i.value} value={i.value}>
                {i.label}
              </ToggleButton>))}
          </ToggleButtonGroup>
        </Box>
        ...
        <Box ref={containerRef} id="tradingview_widget_container" sx={{
            width: width,
            height: height,
            '& iframe': {
                borderRadius: 2,
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2, pt: 2, borderTop: `1px solid ${theme === 'dark' ? '#2a2a3e' : '#e0e0e0'}` }}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Typography variant="caption" component="a" href="https://www.tradingview.com/" target="_blank" sx={{ color: '#ff6b00', textDecoration: 'none', cursor: 'pointer' }}>
              Learn More
            </Typography>
            <Typography variant="caption" component="a" href="https://www.tradingview.com/symbols/" target="_blank" sx={{ color: '#ff6b00', textDecoration: 'none', cursor: 'pointer' }}>
              Full Chart
            </Typography>
          </Box>
        </Box>
...
//# sourceMappingURL=TradingViewWidget.jsx.map