...
import { Box, Typography, Card, CardContent, Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, IconButton, Select, MenuItem, FormControl, Dialog, DialogTitle, DialogContent, DialogActions, Tabs, Tab, Snackbar, Alert, CircularProgress, InputAdornment } from '@mui/material';
import { Add, Remove, Lock, LockOpen, Refresh } from '@mui/icons-material';
// ============================================================================
// Constants
// ============================================================================
const CHAIN_CONFIG = {
...
function generateUserStakes(pools) {
    const stakes = [];
...
export default function FarmingPage() {
    ...
    const handleClaim = async (poolId) => {
        ...
        if (!stake || parseFloat(stake.pendingRewards) <= 0)
            return;
        try {
            await new Promise(resolve => setTimeout(resolve, 1000));
            setUserStakes(prev => prev.map(s => s.poolId === poolId ? { ...s, pendingRewards: '0', lastClaimTime: Date.now() } : s));
    ...
    return (<Box sx={{ minHeight: '100vh', bgcolor: '#0a0a14', p: 3 }}>
      <Box sx={{ maxWidth: 1600, mx: 'auto' }}>
          ...
          <CardContent sx={{ p: 3 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
              <FormControl size="small" sx={{ minWidth: 150 }}>
                <Select value={filterChain} onChange={(e) => setFilterChain(e.target.value)} sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
        ...
        <TableContainer>
                ...
                return (<TableRow key={pool.id} sx={{ '&:hover': { bgcolor: '#2a2a3e' } }}>
                          <TableCell>
                          ...
                          <TableCell>
                            <Chip label={pool.protocol} size="small" sx={{ bgcolor: '#2a2a3e', color: '#9ca3af' }}/>
                          </TableCell>
                          ...
                          <TableCell align="right">
                            <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                              <Button size="small" variant="contained" startIcon={<Add />} onClick={() => { setSelectedPool(pool); setShowStakeDialog(true); }} sx={{ bgcolor: '#00d4aa', color: 'black', minWidth: 0, px: 1 }}>
                                Stake
                              </Button>
                              {userStake && (<>
                                  <Button size="small" variant="outlined" startIcon={<Remove />} onClick={() => { setSelectedPool(pool); setShowUnstakeDialog(true); }} sx={{ borderColor: '#3a3a4e', color: 'white', minWidth: 0, px: 1 }}>
                                    Unstake
                                  </Button>
                                  {parseFloat(userStake.pendingRewards) > 0 && (<Button size="small" variant="contained" onClick={() => handleClaim(pool.id)} sx={{ bgcolor: '#ff9800', color: 'black', minWidth: 0, px: 1 }}>
                                      Claim
                                    </Button>)}
                                </>)}
                            </Box>
              ...
              </TableContainer>) : activeTab === 1 ? (
        /* My Positions */
        <Box>
                {userStakes.length === 0 ? (<Box sx={{ textAlign: 'center', py: 5 }}>
                    <Typography sx={{ color: '#9ca3af', mb: 2 }}>No active staking positions</Typography>
                    <Button variant="contained" startIcon={<Add />} onClick={() => setActiveTab(0)} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
                      Find a Farm
                    </Button>
                  </Box>) : (<TableContainer>
                    <Table>
                    ...
                    if (!pool)
                        return null;
                    return (<TableRow key={stake.poolId} sx={{ '&:hover': { bgcolor: '#2a2a3e' } }}>
                              <TableCell>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  <Typography sx={{ fontSize: 24 }}>{pool.pairIcon}</Typography>
                                  <Typography sx={{ color: '#00d4aa', fontWeight: 'bold' }}>{pool.pair}</Typography>
                              ...
                              <TableCell align="right">
                                {stake.isLocked && stake.unlockTime ? (<Chip icon={<Lock sx={{ fontSize: 16 }}/>} label={timeUntil(stake.unlockTime)} size="small" sx={{ bgcolor: '#ff572220', color: '#ff5722' }}/>) : (<Chip icon={<LockOpen sx={{ fontSize: 16 }}/>} label="LockOpened" size="small" sx={{ bgcolor: '#00d4aa20', color: '#00d4aa' }}/>)}
                              </TableCell>
                              <TableCell align="right">
                                <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                                  <Button size="small" variant="contained" disabled={parseFloat(stake.pendingRewards) <= 0} onClick={() => handleClaim(pool.id)} sx={{ bgcolor: '#ff9800', color: 'black' }}>
                                    Claim
                                  </Button>
                                  <Button size="small" variant="outlined" disabled={stake.isLocked} onClick={() => { setSelectedPool(pool); setShowUnstakeDialog(true); }} sx={{ borderColor: '#3a3a4e', color: 'white' }}>
                                    Unstake
                                  </Button>
                                </Box>
                              </TableCell>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 3 }}>
                {filteredPools.slice(0, 5).map((pool) => (<Card key={pool.id} sx={{ bgcolor: '#2a2a3e', borderRadius: 3, border: '1px solid #00d4aa30' }}>
                    <CardContent>
                      ...
                      <Button fullWidth variant="contained" startIcon={<Add />} onClick={() => { setSelectedPool(pool); setShowStakeDialog(true); }} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
                        Stake {pool.pair}
                      </Button>
                    </CardContent>
                  </Card>))}
      ...
      {/* Stake Dialog */}
      <Dialog open={showStakeDialog} onClose={() => setShowStakeDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#1a1a2e', backgroundImage: 'none' } }}>
        ...
        <DialogContent>
          {selectedPool && (<Box sx={{ mt: 2 }}>
              ...
              <TextField fullWidth type="number" label="Amount to Stake" value={stakeAmount} onChange={(e) => setStakeAmount(e.target.value)} InputProps={{
              ...
              {selectedPool.lockPeriod > 0 && (<Box sx={{ mb: 3 }}>
                  <Typography sx={{ color: '#9ca3af', mb: 2 }}>Lock Period (Higher multiplier)</Typography>
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    {LOCK_PERIODS.map(period => (<Chip key={period.days} label={`${period.label} (${period.multiplier}x)`} onClick={() => setSelectedLockPeriod(period.days)} sx={{
        ...
        <DialogActions sx={{ p: 3 }}>
          ...
          <Button variant="contained" onClick={handleStake} disabled={staking || !stakeAmount || parseFloat(stakeAmount) <= 0} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
      ...
      {/* Unstake Dialog */}
      <Dialog open={showUnstakeDialog} onClose={() => setShowUnstakeDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#1a1a2e', backgroundImage: 'none' } }}>
        <DialogTitle sx={{ color: 'white', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <DialogContent>
          {selectedPool && (<Box sx={{ mt: 2 }}>
              ...
              <TextField fullWidth type="number" label="Amount to Unstake" value={unstakeAmount} onChange={(e) => setUnstakeAmount(e.target.value)} InputProps={{
                endAdornment: <InputAdornment sx={{ color: '#9ca3af' }}>LP</InputAdornment>,
            }} sx={{
                mb: 3,
                '& .MuiInputLabel-root': { color: '#9ca3af' },
        ...
        <DialogActions sx={{ p: 3 }}>
          ...
          <Button variant="contained" onClick={handleUnstake} disabled={staking || !unstakeAmount || parseFloat(unstakeAmount) <= 0} sx={{ bgcolor: '#ff5722', color: 'white' }}>
