...
import { Box, Typography, Card, CardContent, Button, TextField, Chip, CircularProgress, Snackbar, Alert, LinearProgress, Tabs, Tab } from '@mui/material';
import { RocketLaunch, Timer, TrendingUp, People, Ballot, AccessTime, CheckCircle, Cancel } from '@mui/icons-material';
// ============================================================================
// Mock Data
// ============================================================================
...
export default function LaunchpadPage() {
    ...
    const handlePurchase = async () => {
        if (!selectedProject || !allocationAmount)
            return;
        setPurchasing(true);
        try {
    ...
    const renderStatusChip = (status) => {
        ...
        return (<Chip icon={<Icon sx={{ fontSize: 14 }}/>} label={config.label} sx={{
                bgcolor: `${config.color}20`,
                color: config.color,
                fontWeight: 'bold'
            }}/>);
    };
    const renderProgress = (raised, hardCap) => {
    ...
    const renderProjectCard = (project) => (<Card key={project.id} sx={{
            bgcolor: '#2a2a3e',
            borderRadius: 3,
            cursor: 'pointer',
      ...
      <CardContent sx={{ p: 3 }}>
        ...
        {project.status !== 'upcoming' && (<Box sx={{ mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
          {project.kycRequired && (<Chip label="KYC Required" size="small" sx={{ bgcolor: '#ff980020', color: '#ff9800', fontSize: '0.65rem' }}/>)}
          {project.auditStatus === 'completed' && (<Chip label="✅ Audited" size="small" sx={{ bgcolor: '#00d4aa20', color: '#00d4aa', fontSize: '0.65rem' }}/>)}
          {project.vestingPercent > 0 && (<Chip label={`${project.vestingPercent}% TGE`} size="small" sx={{ bgcolor: '#00d4ff20', color: '#00d4ff', fontSize: '0.65rem' }}/>)}
        </Box>

        {project.status === 'live' && (<Button fullWidth variant="contained" sx={{
                mt: 2,
                bgcolor: '#00d4aa',
                color: 'black',
                fontWeight: 'bold',
        ...
        {project.status === 'upcoming' && (<Button fullWidth variant="outlined" sx={{
                mt: 2,
                borderColor: '#00d4aa',
                color: '#00d4aa',
    ...
    return (<Box sx={{ minHeight: '100vh', bgcolor: '#0a0a14', p: 3 }}>
      <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
        ...
        <Card sx={{ bgcolor: '#1a1a2e', borderRadius: 3 }}>
          ...
          <CardContent sx={{ p: 3 }}>
            {loading ? (<Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}>
                <CircularProgress sx={{ color: '#00d4aa' }}/>
              </Box>) : (<Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: 3 }}>
                ...
                (activeTab === 2 && completedProjects.length === 0)) && (<Box sx={{ gridColumn: '1 / -1', textAlign: 'center', py: 8 }}>
      ...
      {selectedProject && (<Box sx={{
          ...
          <Card sx={{
            ...
            <CardContent sx={{ p: 4 }}>
              ...
              {/* Progress */}
              {selectedProject.status !== 'upcoming' && (<Box sx={{ mb: 3 }}>
              ...
              {/* Vesting */}
              {selectedProject.vestingPercent > 0 && (<Box sx={{ bgcolor: '#2a2a3e', p: 2, borderRadius: 2, mb: 3 }}>
                  <Typography variant="caption" sx={{ color: '#9ca3af', mb: 1, display: 'block' }}>
                    Vesting Schedule
                  </Typography>
              ...
              {/* Purchase Form */}
              {selectedProject.status === 'live' && (<Box>
                  <TextField fullWidth type="number" label="Allocation Amount (USDC)" value={allocationAmount} onChange={(e) => setAllocationAmount(e.target.value)} sx={{
                    mb: 2,
                    '& .MuiInputLabel-root': { color: '#9ca3af' },
                    '& input': { color: 'white' },
                    '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: '#3a3a4e' } }
                }}/>
                  <Button fullWidth variant="contained" size="large" disabled={purchasing || !allocationAmount} onClick={handlePurchase} sx={{
                    bgcolor: '#00d4aa',
                    color: 'black',
                    fontWeight: 'bold',
                    py: 1.5,
