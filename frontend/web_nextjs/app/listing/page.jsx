...
export default function TokenListingPage() {
    ...
    return (<div style={{ background: bgPrimary, minHeight: '100vh', color: textPrimary }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 0' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 32 }}>
          {[1, 2, 3].map(s => (<div key={s} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {/* Step 1: Token Info */}
        {step === 1 && (<div style={{ background: 'rgba(30,41,59,0.8)', borderRadius: 16, padding: 32 }}>
            <h2>Token Information</h2>
            
            <div style={{ marginBottom: 24 }}>
              <label style={{ display: 'block', marginBottom: 8, color: '#94a3b8' }}>Chain</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {chains.map(chain => (<div key={chain.id} onClick={() => setToken({ ...token, chainId: chain.id })} style={{ padding: '12px 24px', borderRadius: 8, border: `1px solid ${token.chainId === chain.id ? '#f97316' : 'rgba(255,255,255,0.2)'}`, background: token.chainId === chain.id ? 'rgba(249,115,22,0.2)' : 'transparent', cursor: 'pointer' }}>
                    {chain.name}
                  </div>))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {quoteTokens.map(qt => (<div key={qt} onClick={() => setQuoteToken(qt)} style={{ padding: '12px 24px', borderRadius: 8, border: `1px solid ${quoteToken === qt ? '#f97316' : 'rgba(255,255,255,0.2)'}`, background: quoteToken === qt ? 'rgba(249,115,22,0.2)' : 'transparent', cursor: 'pointer' }}>
                    {qt}
                  </div>))}
              </div>
            </div>
        ...
        {step === 2 && (<div style={{ background: 'rgba(30,41,59,0.8)', borderRadius: 16, padding: 32 }}>
            <h2>Select Listing Tier</h2>
            <p style={{ color: '#94a3b8', marginBottom: 24 }}>Choose the tier that best fits your token.</p>

            <div style={{ display: 'grid', gap: 16 }}>
              {tiers.map(tier => (<div key={tier.id} onClick={() => setSelectedTier(tier.id)} style={{ padding: 24, borderRadius: 12, border: `2px solid ${selectedTier === tier.id ? '#f97316' : 'rgba(255,255,255,0.1)'}`, background: selectedTier === tier.id ? 'rgba(249,115,22,0.1)' : 'transparent', cursor: 'pointer' }}>
        ...
        {/* Step 3: Review */}
        {step === 3 && (<div style={{ background: 'rgba(30,41,59,0.8)', borderRadius: 16, padding: 32 }}>
            <h2>Review & Submit</h2>
