...
import { Box, Typography, Card, CardContent, Button, TextField, Chip, Alert, IconButton, InputAdornment, Slider, Divider, Stack, Switch, FormControlLabel, Dialog, DialogTitle, DialogContent, List, ListItemIcon, ListItemText, ListItemButton, Avatar, Snackbar, LinearProgress, Tooltip, useTheme as useMuiTheme } from '@mui/material';
import { SwapHoriz, Settings, OpenInNew, Shield, Speed, CompareArrows, Refresh, ArrowForward, KeyboardArrowDown, Close } from '@mui/icons-material';
import { useTheme } from '../components/ThemeProvider';
import { TigerSwapWallet, COMMON_TOKENS, DEX_ROUTERS } from '../lib/TigerSwapWallet';
// ============================================================================
// Constants
// ============================================================================
const CHAIN_CONFIG = {
...
export default function SwapPage() {
    ...
    const [gasPrice, setGasPrice] = useState({ slow: 20, standard: 35, fast: 50, instant: 75, baseFee: 30 });
    const [txState, setTxState] = useState({ status: 'idle', hash: null, error: null });
    const [quote, setQuote] = useState(null);
    const [loadingQuote, setLoadingQuote] = useState(false);
    const [priceFromChainlink, setPriceFromChainlink] = useState(null);
    ...
    const SearchIcon = (props) => (<svg {...props} viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
    ...
    useEffect(() => {
        ...
        Object.values(chainTokens).forEach((t) => {
            tokens.push({
                address: t.address,
                symbol: t.symbol,
                name: t.name,
    ...
    // ============================================================================
    const loadTokenBalances = async (w) => {
        if (!w.isConnected())
            return;
    ...
    // ============================================================================
    const loadGasPrice = async (w) => {
        if (!w.isConnected())
            return;
        try {
            const gasInfo = await w.getGasPrice();
            setGasPrice({
    ...
    // ============================================================================
    const loadChainlinkPrice = async (w, baseSymbol) => {
        try {
    ...
    const calculateQuote = useCallback(async () => {
        ...
        try {
            ...
            const amounts = await w.callContract(router, [
                {
                    name: 'getAmountsOut',
            ...
            ], 'getAmountsOut', [amountInRaw.toString(), path]);
    ...
    // ============================================================================
    const calculatePriceImpact = (amountIn, amountOut, decimalsIn, decimalsOut) => {
    ...
    const getTokenBalance = (token) => {
        const balance = tokenBalances.find(b => b.token.address.toLowerCase() === token.address.toLowerCase());
        ...
        try {
            ...
            if (!tokenIn.isNative) {
                ...
                if (BigInt(allowance) < amountInRaw) {
                    const approveTx = await w.approve(tokenIn.address, router, BigInt('0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'));
                    setSnackbar({
                        open: true,
            ...
            if (tokenIn.isNative) {
                ...
                if (weth) {
                    txHash = await w.sendTransaction({
                        ...
                        data: w.encodeFunctionCall([{ name: 'swapExactETHForTokens', inputs: [], outputs: [], stateMutability: 'payable', type: 'function' }], 'swapExactETHForTokens', []),
                    });
                }
            }
            else {
                txHash = await w.executeSwap(tokenIn.address, tokenOut.address, amountInRaw, amountOutMinRaw, path, router, deadlineTimestamp);
            }
    ...
    const renderTokenSelector = () => {
        ...
        return (<Dialog open={!!showTokenSelector} onClose={() => { setShowTokenSelector(null); setSearchQuery(''); }} PaperProps={{
                sx: {
                    bgcolor: '#1a1a2e',
        ...
        <DialogContent>
          <TextField fullWidth placeholder="Search by name, symbol, or address" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} sx={{
                mb: 2,
                input: { color: 'white' },
          ...
          {popularTokens.length > 0 && (<>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
                {popularTokens.map(token => (<Chip key={token.address} label={token.symbol} onClick={() => handleSelectToken(token)} sx={{
                        bgcolor: '#2a2a3e',
                        color: 'white',
                        cursor: 'pointer',
          ...
          <List sx={{ maxHeight: 300, overflow: 'auto' }}>
            {otherTokens.map(token => (<ListItemButton key={token.address} onClick={() => handleSelectToken(token)} sx={{ borderRadius: 1, mb: 0.5 }}>
                <ListItemIcon>
                  {token.logoURI ? (<Avatar src={token.logoURI} sx={{ width: 32, height: 32 }}/>) : (<Avatar sx={{ bgcolor: '#2a2a3e', width: 32, height: 32 }}>
                      {token.symbol[0]}
                    </Avatar>)}
                </ListItemIcon>
                <ListItemText primary={<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    </Box>} secondary={token.name} primaryTypographyProps={{ color: 'white' }} secondaryTypographyProps={{ color: 'gray' }}/>
                <Box sx={{ textAlign: 'right' }}>
                  <Typography sx={{ color: 'white' }}>
                    {formatBalance(getTokenBalance(token), token.decimals)}
                  </Typography>
                  {token.priceUSD && (<Typography variant="caption" sx={{ color: 'gray' }}>
    ...
    return (<Box sx={{
      ...
      <Box sx={{ maxWidth: 480, mx: 'auto' }}>
          ...
          <CardContent sx={{ p: 3 }}>
            ...
            <Box sx={{ mb: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                {wallet.isConnected && tokenIn && (<Typography variant="caption" sx={{ color: 'gray', cursor: 'pointer' }} onClick={() => setMaxAmount()}>
                    Balance: {getMaxBalance()}
                  </Typography>)}
              </Box>
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <TextField fullWidth placeholder="0.0" value={amountIn} onChange={(e) => {
            const val = e.target.value;
            if (/^\d*\.?\d*$/.test(val)) {
                setAmountIn(val);
            }
        }} disabled={!wallet.isConnected} sx={{
            input: {
                color: 'white',
                fontSize: '1.5rem',
                fontWeight: 600,
            ...
            '& .MuiOutlinedInput-root': {
                ...
                <Button onClick={() => setShowTokenSelector('in')} disabled={!wallet.isConnected} sx={{
            bgcolor: '#2a2a3e',
            color: 'white',
            minWidth: 120,
            borderRadius: 2,
            ...
            '&:hover': { bgcolor: '#3a3a4e' }
                  ...
                  {tokenIn ? (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {tokenIn && tokenIn.priceUSD && amountIn && (<Typography variant="caption" sx={{ color: 'gray', mt: 0.5, display: 'block' }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'center', my: -1, position: 'relative', zIndex: 1 }}>
              <IconButton onClick={switchTokens} sx={{
            bgcolor: '#2a2a3e',
            ...
            <Box sx={{ mt: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                {wallet.isConnected && tokenOut && (<Typography variant="caption" sx={{ color: 'gray' }}>
                    Balance: {formatBalance(getTokenBalance(tokenOut), tokenOut.decimals)}
                  </Typography>)}
              </Box>
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <Box sx={{ flex: 1, position: 'relative' }}>
                  <TextField fullWidth placeholder="0.0" value={amountOut} InputProps={{ readOnly: true }} sx={{
            input: {
                color: 'white',
                fontSize: '1.5rem',
                fontWeight: 600,
            ...
            '& .MuiOutlinedInput-root': {
                '& fieldset': { borderColor: 'transparent' },
                  ...
                  {loadingQuote && (<LinearProgress sx={{
                position: 'absolute',
                bottom: 0,
                left: 0,
                right: 0,
                ...
                <Button onClick={() => setShowTokenSelector('out')} disabled={!wallet.isConnected} sx={{
            bgcolor: '#2a2a3e',
            color: 'white',
            minWidth: 120,
            borderRadius: 2,
            ...
            '&:hover': { bgcolor: '#3a3a4e' }
                  ...
                  {tokenOut ? (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {tokenOut && tokenOut.priceUSD && amountOut && (<Typography variant="caption" sx={{ color: 'gray', mt: 0.5, display: 'block' }}>
                  ≈ {formatUSD(parseFloat(amountOut) * tokenOut.priceUSD)}
                </Typography>)}
            </Box>

            {/* Route Details */}
            {quote && quote.route.length > 0 && (<Box sx={{ mt: 2 }}>
                <Button onClick={() => setShowRouteDetails(!showRouteDetails)} sx={{
                color: 'gray',
                textTransform: 'none',
                fontSize: '0.75rem'
                ...
                {showRouteDetails && (<Card sx={{ bgcolor: '#0a0a14', mt: 1, borderRadius: 2 }}>
                    <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                      {quote.route.map((r, i) => (<Box key={i} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
            ...
            <Box sx={{ mt: 3 }}>
              {!wallet.isConnected ? (<Stack spacing={1}>
                  <Button fullWidth variant="contained" onClick={() => connectWallet('metamask')} sx={{
                bgcolor: '#FF007A',
                color: 'white',
                py: 1.5,
                fontWeight: 600,
                '&:hover': { bgcolor: '#cc005a' }
                  ...
                  <Button fullWidth variant="outlined" onClick={() => connectWallet('walletconnect')} sx={{
                borderColor: '#3a3a4e',
                color: 'white',
                py: 1.5,
                ...
                </Stack>) : (<Button fullWidth variant="contained" onClick={executeSwap} disabled={!tokenIn ||
                !tokenOut ||
                !amountIn ||
                parseFloat(amountIn) <= 0 ||
                txState.status === 'approving' ||
                txState.status === 'swapping' ||
                txState.status === 'confirming'} sx={{
                bgcolor: '#00d4ff',
                color: 'black',
                py: 1.5,
                fontWeight: 600,
            ...
            {/* Transaction Hash */}
            {txState.hash && (<Box sx={{ mt: 2, textAlign: 'center' }}>
                <Button href={`${CHAIN_CONFIG[wallet.chainId]?.explorer}/tx/${txState.hash}`} target="_blank" rel="noopener" endIcon={<OpenInNew />} sx={{ color: '#00d4aa', textTransform: 'none' }}>
                  View on Explorer
                </Button>
              </Box>)}
          </CardContent>
        ...
        {/* Settings Panel */}
        {showSettings && (<Card sx={{ mt: 2, bgcolor: '#1a1a2e', borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Box sx={{ mb: 3 }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  {[0.1, 0.5, 1.0].map(val => (<Button key={val} size="small" variant={slippage === val ? 'contained' : 'outlined'} onClick={() => setSlippage(val)} sx={{
                  ...
                  <TextField size="small" type="number" value={slippage} onChange={(e) => setSlippage(parseFloat(e.target.value) || 0)} InputProps={{
                ...
                <FormControlLabel control={<Switch checked={autoSlippage} onChange={(e) => setAutoSlippage(e.target.checked)} sx={{ '& .MuiSwitch-thumb': { bgcolor: '#00d4ff' } }}/>} label={<Typography variant="caption" sx={{ color: 'gray' }}>Auto slippage</Typography>} sx={{ mt: 1 }}/>
              </Box>
              ...
              <Box sx={{ mb: 3 }}>
                ...
                <Slider value={deadline} onChange={(_, v) => setDeadline(v)} min={1} max={60} sx={{ color: '#00d4ff' }}/>
              </Box>
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  {['slow', 'standard', 'fast', 'instant'].map(speed => (<Button key={speed} size="small" variant={gasPreference === speed ? 'contained' : 'outlined'} onClick={() => setGasPreference(speed)} sx={{
                    flex: 1,
                    minWidth: 0,
