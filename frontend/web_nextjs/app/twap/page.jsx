...
import { Box, Typography, Card, CardContent, Button, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Chip, IconButton, Select, MenuItem, FormControl, InputLabel, Dialog, DialogTitle, DialogContent, DialogActions, Tabs, Tab, LinearProgress, Snackbar, Alert, CircularProgress, Slider, InputAdornment, ToggleButton, ToggleButtonGroup } from '@mui/material';
import { TrendingDown, Add, PlayArrow, Pause, Delete, Refresh, CheckCircle } from '@mui/icons-material';
// ============================================================================
// Constants
// ============================================================================
const TOKENS = [
...
export default function TWAPPage() {
    // State
    const [twapOrders, setTwapOrders] = useState([]);
    const [dcaOrders, setDcaOrders] = useState([]);
    const [executionHistory, setExecutionHistory] = useState([]);
    const [loading, setLoading] = useState(true);
    const [activeTab, setActiveTab] = useState(0);
    const [showCreateDialog, setShowCreateDialog] = useState(false);
    const [orderType, setOrderType] = useState('twap');
    // Create form state
    ...
    const handlePauseResume = async (orderId, type, currentStatus) => {
        const newStatus = currentStatus === 'active' ? 'paused' : 'active';
        try {
            if (type === 'twap') {
                setTwapOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: newStatus } : o));
            }
            else {
                setDcaOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: newStatus } : o));
            }
    ...
    const handleCancel = async (orderId, type) => {
        try {
            if (type === 'twap') {
                setTwapOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: 'cancelled' } : o));
            }
            else {
                setDcaOrders(prev => prev.map(o => o.id === orderId ? { ...o, status: 'cancelled' } : o));
            }
    ...
    // Main Render
    // ============================================================================
    return (<Box sx={{ minHeight: '100vh', bgcolor: '#0a0a14', p: 3 }}>
      <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Button variant="outlined" startIcon={<Refresh />} onClick={loadData} sx={{ borderColor: '#3a3a4e', color: 'white' }}>
              Refresh
            </Button>
            <Button variant="contained" startIcon={<Add />} onClick={() => setShowCreateDialog(true)} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
              Create Order
            </Button>
          </Box>
        </Box>
        ...
        /* TWAP Orders */
        <Box>
                {twapOrders.length === 0 ? (<Box sx={{ textAlign: 'center', py: 5 }}>
                    <Typography sx={{ color: '#9ca3af', mb: 2 }}>No TWAP orders yet</Typography>
                    <Button variant="contained" startIcon={<Add />} onClick={() => { setOrderType('twap'); setShowCreateDialog(true); }} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {twapOrders.map(order => (<TableRow key={order.id} sx={{ '&:hover': { bgcolor: '#2a2a3e' } }}>
                            <TableCell>
                            ...
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <LinearProgress variant="determinate" value={(parseFloat(order.filledAmount) / parseFloat(order.totalAmount)) * 100} sx={{ width: 80, height: 6, borderRadius: 3, bgcolor: '#2a2a3e', '& .MuiLinearProgress-bar': { bgcolor: '#00d4aa' } }}/>
                        ...
                        color: order.status === 'active' ? '#00d4aa' : '#ff9800',
                            ...
                            <TableCell align="right">
                              <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                                <IconButton size="small" onClick={() => handlePauseResume(order.id, 'twap', order.status)} sx={{ color: order.status === 'active' ? '#ff9800' : '#00d4aa' }}>
                                  {order.status === 'active' ? <Pause fontSize="small"/> : <PlayArrow fontSize="small"/>}
                                </IconButton>
                                <IconButton size="small" onClick={() => handleCancel(order.id, 'twap')} sx={{ color: '#ff5722' }}>
        ...
        /* DCA Orders */
        <Box>
                {dcaOrders.length === 0 ? (<Box sx={{ textAlign: 'center', py: 5 }}>
                    <Typography sx={{ color: '#9ca3af', mb: 2 }}>No DCA orders yet</Typography>
                    <Button variant="contained" startIcon={<Add />} onClick={() => { setOrderType('dca'); setShowCreateDialog(true); }} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {dcaOrders.map(order => (<TableRow key={order.id} sx={{ '&:hover': { bgcolor: '#2a2a3e' } }}>
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                <Typography sx={{ color: '#00d4aa', fontWeight: 'bold' }}>{order.tokenIn}</Typography>
                                <TrendingDown sx={{ color: '#9ca3af', fontSize: 16 }}/>
                        ...
                        color: order.status === 'active' ? '#00d4aa' : '#ff9800',
                            ...
                            <TableCell align="right">
                              <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                                <IconButton size="small" onClick={() => handlePauseResume(order.id, 'dca', order.status)} sx={{ color: order.status === 'active' ? '#ff9800' : '#00d4aa' }}>
                                  {order.status === 'active' ? <Pause fontSize="small"/> : <PlayArrow fontSize="small"/>}
                                </IconButton>
                                <IconButton size="small" onClick={() => handleCancel(order.id, 'dca')} sx={{ color: '#ff5722' }}>
                                  <Delete fontSize="small"/>
                                </IconButton>
                              </Box>
                            </TableCell>
        ...
        <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {executionHistory.map(exec => (<TableRow key={exec.id} sx={{ '&:hover': { bgcolor: '#2a2a3e' } }}>
                        <TableCell>
                          <Typography variant="caption" sx={{ color: '#9ca3af' }}>
                        ...
                        <TableCell>
                          <Chip icon={exec.status === 'success' ? <CheckCircle sx={{ fontSize: 14 }}/> : undefined} label={exec.status} size="small" sx={{
      ...
      {/* Create Order Dialog */}
      <Dialog open={showCreateDialog} onClose={() => setShowCreateDialog(false)} maxWidth="md" fullWidth PaperProps={{ sx: { bgcolor: '#1a1a2e', backgroundImage: 'none' } }}>
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            {/* Order Type Toggle */}
            <Box sx={{ mb: 3 }}>
              <ToggleButtonGroup value={orderType} exclusive onChange={(_, v) => v && setOrderType(v)} fullWidth sx={{ '& .MuiToggleButton-root': { color: '#9ca3af', borderColor: '#3a3a4e' }, '& .Mui-selected': { bgcolor: '#00d4aa', color: 'black' } }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mb: 3 }}>
              <FormControl fullWidth size="small">
                ...
                <Select value={tokenIn} onChange={(e) => setTokenIn(e.target.value)} label="From Token" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                  {TOKENS.map(t => (<MenuItem key={t.symbol} value={t.symbol}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <span>{t.icon}</span>
                        <span>{t.symbol}</span>
                      </Box>
              ...
              <FormControl fullWidth size="small">
                ...
                <Select value={tokenOut} onChange={(e) => setTokenOut(e.target.value)} label="To Token" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                  {TOKENS.map(t => (<MenuItem key={t.symbol} value={t.symbol}>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <span>{t.icon}</span>
                        <span>{t.symbol}</span>
            ...
            {/* Amount */}
            <TextField fullWidth type="number" label="Total Amount" value={totalAmount} onChange={(e) => setTotalAmount(e.target.value)} InputProps={{
            ...
            {orderType === 'twap' ? (<Box>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mb: 3 }}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select value={numOrders} onChange={(e) => setNumOrders(e.target.value)} label="Number of Orders" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                      {[5, 10, 20, 50, 100].map(n => (<MenuItem key={n} value={n}>{n} orders</MenuItem>))}
                    </Select>
                  </FormControl>
                  <FormControl fullWidth size="small">
                    <InputLabel sx={{ color: '#9ca3af' }}>Interval</InputLabel>
                    <Select value={intervalMinutes} onChange={(e) => setIntervalMinutes(e.target.value)} label="Interval" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                      {INTERVALS.map(int => (<MenuItem key={int.value} value={int.value}>{int.label}</MenuItem>))}
                    </Select>
                  </FormControl>
                </Box>
                ...
                <FormControl fullWidth size="small" sx={{ mb: 3 }}>
                  ...
                  <Select value={priceType} onChange={(e) => setPriceType(e.target.value)} label="Price Type" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                    <MenuItem value="market">Market Price</MenuItem>
                    <MenuItem value="limit">Limit Price</MenuItem>
                  </Select>
                </FormControl>

                {priceType === 'limit' && (<TextField fullWidth type="number" label="Limit Price" value={limitPrice} onChange={(e) => setLimitPrice(e.target.value)} sx={{ mb: 3, '& .MuiInputLabel-root': { color: '#9ca3af' }, '& input': { color: 'white' }, '& .MuiOutlinedInput-root': { '& fieldset': { borderColor: '#3a3a4e' } } }}/>)}
              </Box>) : (
        /* DCA Settings */
        <Box>
                <FormControl fullWidth size="small" sx={{ mb: 3 }}>
                  <InputLabel sx={{ color: '#9ca3af' }}>Frequency</InputLabel>
                  <Select value={frequency} onChange={(e) => setFrequency(e.target.value)} label="Frequency" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                    {FREQUENCIES.map(f => (<MenuItem key={f.value} value={f.value}>{f.label}</MenuItem>))}
                  </Select>
                </FormControl>

                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, mb: 3 }}>
                  {frequency === 'weekly' && (<FormControl fullWidth size="small">
                      <InputLabel sx={{ color: '#9ca3af' }}>Day of Week</InputLabel>
                      <Select value={dayOfWeek} onChange={(e) => setDayOfWeek(e.target.value)} label="Day of Week" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                        {DAYS_OF_WEEK.map((day, i) => (<MenuItem key={i} value={i}>{day}</MenuItem>))}
                      </Select>
                    </FormControl>)}
                  {frequency === 'monthly' && (<FormControl fullWidth size="small">
                      <InputLabel sx={{ color: '#9ca3af' }}>Day of Month</InputLabel>
                      <Select value={dayOfMonth} onChange={(e) => setDayOfMonth(e.target.value)} label="Day of Month" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                        {Array.from({ length: 28 }, (_, i) => (<MenuItem key={i + 1} value={i + 1}>{i + 1}</MenuItem>))}
                      </Select>
                    </FormControl>)}
                  <FormControl fullWidth size="small">
                    <InputLabel sx={{ color: '#9ca3af' }}>Time of Day</InputLabel>
                    <Select value={hourOfDay} onChange={(e) => setHourOfDay(e.target.value)} label="Time of Day" sx={{ color: 'white', '& .MuiOutlinedInput-notchedOutline': { borderColor: '#3a3a4e' } }}>
                      {Array.from({ length: 24 }, (_, i) => (<MenuItem key={i} value={i}>{i.toString().padStart(2, '0')}:00</MenuItem>))}
                    </Select>
                  </FormControl>
                </Box>
              </Box>)}
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Slider value={slippageBps} onChange={(_, v) => setSlippageBps(v)} min={10} max={500} step={10} marks={[{ value: 50, label: '0.5%' }, { value: 100, label: '1%' }, { value: 500, label: '5%' }]} sx={{ color: '#00d4aa' }}/>
            </Box>

            {/* Summary */}
            <Box sx={{ bgcolor: '#2a2a3e', p: 2, borderRadius: 2 }}>
              <Typography variant="caption" sx={{ color: '#9ca3af' }}>
                {orderType === 'twap' ? (<>Your order will be split into {numOrders} orders over {INTERVALS.find(i => i.value === intervalMinutes)?.label?.toLowerCase()}.</>) : (<>You will buy approximately {(parseFloat(totalAmount) / (orderType === 'dca' ? numOrders : 1)).toFixed(2)} {tokenOut} per order on a {frequency} basis.</>)}
              </Typography>
            </Box>
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 3 }}>
          ...
          <Button variant="contained" onClick={handleCreateOrder} disabled={creating || !totalAmount} sx={{ bgcolor: '#00d4aa', color: 'black' }}>
      ...
      <Snackbar open={snackbar.open} autoHideDuration={5000} onClose={() => setSnackbar({ ...snackbar, open: false })} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
        <Alert onClose={() => setSnackbar({ ...snackbar, open: false })} severity={snackbar.severity} sx={{ bgcolor: snackbar.severity === 'success' ? '#1b5e20' : snackbar.severity === 'error' ? '#b71c1c' : '#1a237e' }}>
          {snackbar.message}
        </Alert>
      </Snackbar>
    </Box>);
