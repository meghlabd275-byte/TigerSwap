...
export default function TigerWallet() {
    ...
    const claimAirdrop = (airdropAddress) => {
        setIsLoading(true);
        const newTx = {
            id: Math.random().toString(36).substr(2, 9),
            type: 'airdrop',
    ...
    const renderDashboard = () => (<div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-orange-600 to-orange-800 rounded-xl p-6 text-white">
        ...
        <div className="flex gap-4 mt-4">
          {wallet?.chains.map(chainId => (<div key={chainId} className="bg-white/20 rounded-lg px-3 py-1 text-sm">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        ].map((action, i) => (<button key={i} onClick={action.action} className="bg-gray-800 rounded-xl p-4 text-center hover:bg-gray-700 transition">
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        ...
        <div className="space-y-3">
          {transactions.length === 0 ? (<div className="text-gray-500 text-center py-4">No transactions yet</div>) : (transactions.slice(0, 5).map((tx) => (<div key={tx.id} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
    ...
    const renderSend = () => (<div className="bg-gray-800 rounded-xl p-6">
      <h3 className="text-xl font-semibold text-white mb-6">Send</h3>

      {/* Chain Selector */}
      <div className="mb-4">
        <label className="block text-gray-400 text-sm mb-2">Network</label>
        <select value={selectedChain} onChange={(e) => setSelectedChain(parseInt(e.target.value))} className="w-full bg-gray-700 text-white rounded-lg p-3">
          {DEFAULT_CHAINS.map(chain => (<option key={chain.id} value={chain.id}>{chain.name} ({chain.symbol})</option>))}
        </select>
      </div>
      ...
      <div className="mb-4">
        ...
        <input type="text" value={sendAddress} onChange={(e) => setSendAddress(e.target.value)} placeholder="0x..." className="w-full bg-gray-700 text-white rounded-lg p-3 font-mono"/>
      </div>

      {/* Amount */}
      <div className="mb-6">
        <label className="block text-gray-400 text-sm mb-2">Amount</label>
        <input type="number" value={sendAmount} onChange={(e) => setSendAmount(e.target.value)} placeholder="0.0" className="w-full bg-gray-700 text-white rounded-lg p-3"/>
      </div>

      {/* Send Button */}
      <button onClick={sendTransaction} disabled={isLoading} className="w-full bg-orange-600 text-white rounded-lg py-3 font-bold hover:bg-orange-700 transition disabled:opacity-50">
        {isLoading ? 'Sending...' : 'Send'}
      </button>
    </div>);
    const renderSwap = () => (<div className="bg-gray-800 rounded-xl p-6">
      ...
      <div className="mb-4">
        ...
        <div className="flex gap-2">
          <select value={swapFromToken} onChange={(e) => setSwapFromToken(e.target.value)} className="flex-1 bg-gray-700 text-white rounded-lg p-3">
            <option value="ETH">ETH</option>
            <option value="BTC">BTC</option>
            <option value="USDT">USDT</option>
            <option value="SOL">SOL</option>
          </select>
          <input type="number" value={swapAmount} onChange={(e) => setSwapAmount(e.target.value)} placeholder="0.0" className="flex-1 bg-gray-700 text-white rounded-lg p-3"/>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <select value={swapToToken} onChange={(e) => setSwapToToken(e.target.value)} className="w-full bg-gray-700 text-white rounded-lg p-3">
          <option value="USDT">USDT</option>
      ...
      {/* Swap Button */}
      <button onClick={swapTokens} disabled={isLoading} className="w-full bg-orange-600 text-white rounded-lg py-3 font-bold hover:bg-orange-700 transition disabled:opacity-50">
        {isLoading ? 'Swapping...' : 'Swap'}
      </button>
    </div>);
    const renderEarn = () => (<div className="space-y-6">
      {/* Liquidity Pools */}
      <div className="bg-gray-800 rounded-xl p-6">
        ...
        ].map((pool, i) => (<div key={i} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
              <div>
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        ...
        ].map((airdrop, i) => (<div key={i} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
              <div>
                <div className="text-white font-bold">{airdrop.name}</div>
                <div className="text-gray-400 text-sm">{airdrop.amount}</div>
              </div>
              <button onClick={() => claimAirdrop(airdrop.name)} disabled={!airdrop.claimable} className={`px-4 py-2 rounded-lg font-bold ${airdrop.claimable
    ...
    if (!wallet) {
        ...
        <div className="max-w-md w-full">
          ...
          {!showCreate ? (<div className="space-y-4">
              <button onClick={() => setShowCreate(true)} className="w-full bg-orange-600 text-white rounded-xl py-4 font-bold text-lg hover:bg-orange-700 transition">
                Create New Wallet
              </button>
              <button onClick={() => setShowCreate(true)} className="w-full bg-gray-800 text-white rounded-xl py-4 font-bold text-lg hover:bg-gray-700 transition">
                Import Wallet
              </button>
            </div>) : (<div className="bg-gray-800 rounded-xl p-6 space-y-4">
              <h2 className="text-xl font-bold text-white">Create Wallet</h2>

              <input type="text" value={walletName} onChange={(e) => setWalletName(e.target.value)} placeholder="Wallet Name" className="w-full bg-gray-700 text-white rounded-lg p-3"/>

              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="w-full bg-gray-700 text-white rounded-lg p-3"/>

              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Confirm Password" className="w-full bg-gray-700 text-white rounded-lg p-3"/>

              <button onClick={createWallet} className="w-full bg-orange-600 text-white rounded-lg py-3 font-bold hover:bg-orange-700 transition">
                Create Wallet
              </button>

              {showSeedPhrase && (<div className="mt-4 p-4 bg-yellow-900/50 rounded-lg">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {seedPhrase.map((word, i) => (<div key={i} className="bg-gray-900 text-white text-sm p-2 rounded text-center">
                        {i + 1}. {word}
                      </div>))}
                  </div>
                </div>)}
    ...
    return (<div className="min-h-screen bg-gray-900 text-white">
      ...
      <nav className="bg-gray-800 border-b border-gray-700">
        ...
        ].map((tab) => (<button key={tab.id} onClick={() => setCurrentView(tab.id)} className={`flex-1 py-4 text-center ${currentView === tab.id
...
//# sourceMappingURL=TigerWallet.jsx.map