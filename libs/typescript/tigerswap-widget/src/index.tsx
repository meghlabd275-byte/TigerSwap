...
// Widget Props
export interface TigerSwapWidgetProps {
  config: WidgetConfig;
...
class TigerSwapAPIClient {
  ...
  async getQuote(request: QuoteRequest): Promise<Quote> {
    return this.request<Quote>('/quote', {
      method: 'POST',
  ...
  async getTokens(chainId: number): Promise<Token[]> {
    return this.request<Token[]>(`/tokens/${chainId}`);
  }

  async getChains(): Promise<{ id: number; name: string; logo: string }[]> {
    return this.request('/chains');
  }

  async buildSwap(request: SwapRequest): Promise<{ tx: any; quote: Quote }> {
  ...
  async executeSwap(tx: any): Promise<SwapResult> {
    return this.request<SwapResult>('/swap/execute', {
      method: 'POST',
...
export function useTigerSwapWidget(config: WidgetConfig, callbacks?: WidgetCallbacks) {
...
export function TigerSwapWidget({ config, callbacks, className }: TigerSwapWidgetProps) {
  const {
    tokens,
    quote,
    loading,
    ...
    connectWallet,
    disconnectWallet,
  } = useTigerSwapWidget(config, callbacks);
  ...
  return (
    <div 
      className={`tigerswap-widget ${themeClass} ${className || ''}`}
      style={{
    ...
    >
      ...
      <div className="widget-body" style={{ padding: '16px' }}>
        ...
        <button
          onClick={handleSwap}
          disabled={loading || !quote || !wallet.isConnected}
          style={{
            width: '100%',
            padding: '16px',
            borderRadius: '12px',
...
export type { WidgetConfig, WidgetCallbacks, Token, Quote, SwapResult, WalletState };

// Utility function to embed widget
export function embedTigerSwapWidget(
